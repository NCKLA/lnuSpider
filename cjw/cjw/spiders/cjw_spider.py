...
class CjwSpiderSpider(CrawlSpider):
    ...
    start_urls = ['http://finance.caijing.com.cn/bank/']

    rules = (
        Rule(LinkExtractor(allow=r'http:\/\/finance.caijing.com.cn\/.+'), follow=True),
        Rule(LinkExtractor(allow=r"http:\/\/finance.caijing.com.cn\/[0-9]+\/[0-9]+\.shtml"), callback="parse_item", follow=False)
    )
    ...
    def parse_item(self, response):
        ...
        # print(article)
        # print('='*30)
        item = CjwItem(title=title, source=source, date=date, article=article, url=response.url)
        yield item