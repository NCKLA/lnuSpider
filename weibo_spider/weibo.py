...
class Weibo(object):
    def __init__(self, config):
        ...
        self.write_mode = config['write_mode']  # 结果信息保存类型，为list形式，包含csv类型
        ...
        if not isinstance(user_id_list, list):
            ...
            user_config_list = [{'user_id': user_id,'since_date': self.since_date } for user_id in user_id_list]
    ...
    def validate_config(self, config):
        ...
        for mode in config['write_mode']:
            if mode not in write_mode:
                sys.exit(u'%s为无效模式，请从csv、json中挑选一个或多个作为write_mode' %mode)
    ...
    def get_result_headers(self):
        """获取要写入结果文件的表头"""
        result_headers = ['id', 'bid', '正文', '原始图片url', '视频url', '位置', '日期', '工具', '点赞数','评论数', '转发数', '话题', '@用户']
        if not self.filter:
    ...
    def weibo_to_redis(self, wrote_count):
        """将爬取的微博信息写入redis数据库"""
        redis_config = {
            'host': 'localhost',
            'port': 6379,
    ...
    def write_data(self, wrote_count):
        ...
        if self.got_count > wrote_count:
            ...
            if 'redis' in self.write_mode:
                self.weibo_to_redis(wrote_count)
